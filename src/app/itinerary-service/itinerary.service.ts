...
export class ItineraryService {
  static API_KEY = 'AIzaSyATauOvqEKtqfj36Kd2GPCFA3s2GDuNhSo';
  ...
  getEndStation(steps: RouteStep[]): string {
    return steps.findLast
      ? steps.findLast((step: RouteStep) => step.emoji === '🚆' || step.emoji === '🚅')?.title || ''
      : [...steps].reverse().find((step) => step.emoji === '🚆' || step.emoji === '🚅')?.title || '';
  }
